...
void buildSymbolTable(TreeNode* syxTree,SymbolTable* symTable){
    //if node kind is id or read or assign then insert the id in symbol table
    if(syxTree->node_kind==ID_NODE||syxTree->node_kind==READ_NODE||syxTree->node_kind==ASSIGN_NODE){
        symTable->Insert(syxTree->id,syxTree->line_num);
    }
    //iterate over all children in the syxTree
    for (int i = 0; i < MAX_CHILDREN; ++i) {
        if(syxTree->child[i])
            buildSymbolTable(syxTree->child[i],symTable);
    }
    //move to sibling
    if(syxTree->sibling) buildSymbolTable(syxTree->sibling,symTable);
}
void setNodeDataType(TreeNode* syxTree){
    //if node kind is id or num then its data type is integer
    if(syxTree->node_kind==ID_NODE||syxTree->node_kind==NUM_NODE){
        syxTree->expr_data_type=INTEGER;
    }
    else if(syxTree->node_kind==OPER_NODE){
        //if node operator is equal or less than then its data type is boolean otherwise data type is integer
        if(syxTree->oper==EQUAL||syxTree->oper==LESS_THAN){
            syxTree->expr_data_type=BOOLEAN;
        }
        else{
            syxTree->expr_data_type=INTEGER;
        }
    ...
    for (int i = 0; i < MAX_CHILDREN; ++i) {
        if(syxTree->child[i])
            setNodeDataType(syxTree->child[i]);
    }
    //move to sibling
    if(syxTree->sibling) setNodeDataType(syxTree->sibling);
}
void typeChecking(TreeNode* syxTree){
    //if node kind is if condition and the condition data type not equal boolean
    if(syxTree->node_kind==IF_NODE&&syxTree->child[0]->expr_data_type!=BOOLEAN){
        cout<<"Error, an \"if\" condition should be followed by a Boolean."<<endl;
    }
    //if node kind is repeat condition and the condition data type not equal boolean
    if(syxTree->node_kind==REPEAT_NODE&&syxTree->child[1]->expr_data_type!=BOOLEAN){
        cout<<"Error, an \"repeat\" condition should be followed by a Boolean."<<endl;
    }
    //if node kind is operation node and any of two child is not integer
    if(syxTree->node_kind==OPER_NODE){
        if(syxTree->child[0]->expr_data_type!=INTEGER || syxTree->child[1]->expr_data_type!=INTEGER) {
            cout << "Error, Operator applied on integer values only"<<endl;
        }
    }
    //When node kind is write and data type  not equal integer
    if(syxTree->node_kind==WRITE_NODE&&syxTree->child[0]->expr_data_type!=INTEGER){
        cout<<"Error, Write applied on integer values only"<<endl;
    }
    //When node kind is assign and data type not equal integer
    if(syxTree->node_kind==ASSIGN_NODE&&syxTree->child[0]->expr_data_type!=INTEGER){
        cout<<"Error, Assign applied on integer values only"<<endl;
    }

    //iterate over all children in the syxTree
    for (int i = 0; i < MAX_CHILDREN; ++i) {
        if(syxTree->child[i])
            typeChecking(syxTree->child[i]);
    }
    //move to sibling
    if(syxTree->sibling) typeChecking(syxTree->sibling);
}
int* variables;
int getVal(TreeNode* treeNode, SymbolTable* symbolTable)
{
    // If node kind is id , return the value of id
    if(treeNode->node_kind == ID_NODE) {
        int idx=symbolTable->Find(treeNode->id)->memloc;
        return variables[idx];
    }
    // If node kind is num , return value
    if(treeNode->node_kind == NUM_NODE) {
        return treeNode->num;
    }
    //getting the val of left side and right side of the operator and return the of the val of operator
    int leftSide= getVal(treeNode->child[0], symbolTable);
    int rightSide= getVal(treeNode->child[1], symbolTable);
    if(treeNode->oper == POWER) {
        int val=pow(leftSide, rightSide);
        return val;
    }
    if(treeNode->oper == LESS_THAN) {
        int val=leftSide < rightSide;
        return val;
    }
    ...
    if(treeNode->oper == PLUS) {
        int val=leftSide + rightSide;
        return val;
    }
    if(treeNode->oper == MINUS) {
        int val=leftSide - rightSide;
        return val;
    }
    if(treeNode->oper == TIMES) {
        int val=leftSide * rightSide;
        return val;
    }
    if(treeNode->oper == DIVIDE) {
        int val=leftSide / rightSide;
        return val;
    }
    return 0;
}
void simulation(TreeNode* treeNode, SymbolTable* symbolTable)
{
    //if node kind is read , take the input of variable
    if(treeNode->node_kind==READ_NODE)
    {
        cout<<"Enter "<<treeNode->id<<": "<<endl;
        int idx=symbolTable->Find(treeNode->id)->memloc;
        cin>>variables[idx];
    }
    //if node is write , print the variable
    if(treeNode->node_kind==WRITE_NODE)
    {
        int val= getVal(treeNode->child[0], symbolTable);
        cout<<"Value: "<<val<<endl;
    }
    // if node kind is assign
    if(treeNode->node_kind==ASSIGN_NODE)
    {
        //Store val in the variable
        int val= getVal(treeNode->child[0], symbolTable);
        int idx=symbolTable->Find(treeNode->id)->memloc;
        variables[idx]=val;
    }
    // if getVal the first and second child for all tree node
    if(treeNode->node_kind==REPEAT_NODE)
    {
        do {
            simulation(treeNode->child[0], symbolTable);
        }while(!getVal(treeNode->child[1], symbolTable));
    }
    // if node kind is if
    if(treeNode->node_kind==IF_NODE)
    {
        //if the condition achieved
        if(getVal(treeNode->child[0], symbolTable)) {
            simulation(treeNode->child[1], symbolTable);
        }
        else if(treeNode->child[2]) {
            simulation(treeNode->child[2], symbolTable);
        }
    }
    //move to sibling
    if(treeNode->sibling) {
        simulation(treeNode->sibling, symbolTable);
    }
}
int main(){
    CompilerInfo compilerInfo("input.txt","output.txt","debug.txt");
    TreeNode* syntaxTree;
    syntaxTree= Parse(&compilerInfo);
    SymbolTable symbolTable;
    //build symbol table
    buildSymbolTable(syntaxTree,&symbolTable);
    //set the data type for each node in syntax tree
    setNodeDataType(syntaxTree);
    ...
    variables=new int[symbolTable.num_vars];
    simulation(syntaxTree, &symbolTable);
